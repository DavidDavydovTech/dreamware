...
class GameHUD extends Container {
     ...
     */
    constructor({ app, timeMod = 1, timeModMeterMax = 3, lives = 4, livesMax = 4, character = "clingy" }) {
        super();
    ...
    animateLevelNumberCycle = ({ number, fadeInMS = 500, fadeOutMS = 500, displayMS = 2000, x = 400, y = 170 }) => {
        return () =>
            new Promise((resolve) => {
                const { _appReference: app, timeMod } = this;
    ...
    animateZoomCycle = ({ zoomMS = 1000, zoomInScale = 2, x = 400, y = 170, callback, callbackEnd }) => {
        return () =>
            new Promise((resolve) => {
                if (callback) {
                    callback();
